...
class DataProcess:
    ...
    def sync_margin_fee_discount(self):
        ...
        try:
            ...
            margin_fee_sql = """
                select    b.id,
                    round(cast(a.total_margin_fee_num as double), 6) as coin_num,
                    round(cast(if(c.price_usdt = 0 ,a.total_margin_fee_num * e.price_usdt, a.total_margin_fee_num *c.price_usdt)
                    as double), 6) as usd_num
                from (select 
	                currency_trade_id,
	                sum(fee) total_margin_fee_num
                FROM df01.ods_iwala_trade_user
				WHERE toDate(add_time) = '{}' 
				  AND kind = 'margin'
				group by currency_trade_id
                having total_margin_fee_num>0) a
            ...
            result = self.__ch_conn.execute(margin_fee_sql)
            journal_items = []
            if result['code']:
...
def main():
    ...
    func_list = ['sync_margin_fee_discount']
