...
class DataProcess:
    ...
    def sync_spot_activity_cost(self):
        ...
        try:
            # 查询专用账户发放明细 生成 journal item
            spot_activity_cost = """
                select  b.id,
                    round(cast(a.total_currency_activity_cost_num as double), 6) as coin_num,
                    round(cast(a.total_currency_activity_cost_num * c.to_usd_rate as double), 6) as usd_num
                from (select 
	                currency_id,
	                sum(money) total_currency_activity_cost_num
                from df01.ods_iwala_finance
                where type = 247 and member_id =531160 and FROM_UNIXTIME(add_time) >= '{}' and FROM_UNIXTIME(add_time) < '{}'
                group by currency_id
                having total_currency_activity_cost_num>0) a
            ...
            result = self.__ch_conn.execute(spot_activity_cost)
            journal_items = []
            if result['code']:
...
def main():
    ...
    func_list = ['sync_spot_activity_cost']
