...
日期    : 2023/1/3 4:47 PM
作者    : wushuang
功能    :  "入金返佣，实际领取部分 topup rebate unpaid"
参数    : 
"""
...
class DataProcess:
    ...
    def sync_invitation_rebate_received(self):
        ...
        try:
            ...
            invitation_sql = """
                ...
                from df01.ods_iwala_invite_commission_gold
                where receive_status = 1 and update_time >= '{}' and update_time < '{}'
                group by commission_currency_id) a
            ...
            self.log.info("sql execute: {}".format(invitation_sql))
            self.__ch_conn.connect()
            result = self.__ch_conn.execute(invitation_sql)
            journal_items = []
            if result['code']:
...
def main():
    ...
    func_list = ['sync_invitation_rebate_received']
