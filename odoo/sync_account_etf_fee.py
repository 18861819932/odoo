...
class DataProcess:
    ...
    def sync_etf_fee_discount(self):
        ...
        try:
            ...
            etf_fee_sql = """
                     select    b.id,
                    round(cast(a.total_etf_fee_num as double), 6) as coin_num,
                    round(cast(if(c.price_usdt = 0 ,a.total_etf_fee_num * e.price_usdt, a.total_etf_fee_num *c.price_usdt)
                    as double), 6) as usd_num
                from (select 
	                currency_trade_id,
	                sum(fee) total_etf_fee_num
            ...
            result = self.__ch_conn.execute(etf_fee_sql)
            journal_items = []
            if result['code']:
...
def main():
    ...
    func_list = ['sync_etf_fee_discount']
