...
# -*- coding: utf-8 -*-
"""
日期    : 2022/12/11 11:52 PM
作者    : wushuang
功能    : 本平台归集gas费gasfee charged for pooling
参数    : 
"""
    ...
    def sync_pool_gas_fee(self):
        ...
        try:
            ...
            pool_sql = """
                ...
                from (select
                    fee_mark,
                    sum(gas_fee) as gas_fee_amount
                from
                    df01.ods_wallet_consumed_gasfee
                where
                    block_time >= '{}'
                    and block_time < '{}'
                    and transfer_type in (2, 3)
                group by
                    fee_mark) a inner join odoo.dim_odoo_currency b on a.fee_mark = b.full_name 
                where b.stime = '{}' and position(b.name, '-') > 0)
                select 
                    gd.id,
                    round(cast(gas_fee_amount as double), 6) as fee_amount,
                    round(cast(gd.gas_fee_amount * dr.to_usd_rate as double), 6) as usd_gas_fee
            ...
            self.log.info(pool_sql)
            self.__ch_conn.connect()
            result = self.__ch_conn.execute(pool_sql)
            journal_items = []
            if result['code']:
