    ...
    def sync_spot_rebate(self):
        ...
        try:
            ...
            spot_sql = """
                ...
                from (select 
                    currency, sum(transfer_amount) as trans_num
                from df01.ods_ngdesuam_t_transfer_record
                where transfer_time >= toUnixTimestamp('{}') and transfer_time < toUnixTimestamp('{}')
                and transfer_status = 2 and from_account = 2 and to_account = 1 and participant_id not in (
                    select member_id
            ...
            if result['code']:
                ...
                journal_entry = {"date": str(self.stime.date()),
                                 "ref": "user movement contract to spot",
                                 "journal_id": 53,
                                 "company_id": 24,
                                 "currency_id": 2}
