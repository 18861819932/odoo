...
class DataProcess:
    ...
    def sync_account_fund_profit_share_safe_float_interest(self):
        ...
        try:
            ...
            fund_profit_share_safe_float_interest_sql = """
                                    select id,fund_currency_mark,fund_finance_type,coin_num_cur_price,usd_num_cur_price,coin_num_client_asset,usd_num_client_asset,coin_num_company_asset,usd_num_company_asset,
	                                                   coin_num_client_profit,usd_num_client_profit,coin_num_client_manage,usd_num_client_manage,coin_num_float_profit,                  
										...
										from(   select id,fund_currency_mark,fund_finance_type,coin_num_cur_price,usd_num_cur_price,coin_num_client_asset,usd_num_client_asset,coin_num_company_asset,usd_num_company_asset,
			                                    ...
			                                    (usd_num_cur_price+usd_num_float_profi_dr-usd_num_client_asset-usd_num_company_asset-usd_num_client_profit-usd_num_client_manage)   as  usd_num_float_profi_cr
	                                            from (  select  b.id as id,a.fund_currency_mark as fund_currency_mark,
			                                                    ...
			                                                    toDecimal256(-1*(a.manage_fee+a.out_mange_fee), 6) as  coin_num_client_manage,
			                                                    toDecimal256((a.manage_fee+a.out_mange_fee)*c.price_usdt, 6) as  usd_num_client_manage,
            ...
            result = self.__ch_conn.execute(fund_profit_share_safe_float_interest_sql)
            print(fund_profit_share_safe_float_interest_sql)
            print(result['data'])
            if result['code']:
                ...
                for item in result['data']:
                    ...
                    journal_entry = {"date": str(self.stime.date()),
                                 "ref": "<%s> Float Rate Guaranteed Fund return"%(item[1]),
                                 "journal_id": 56,
                                 "company_id": 24,
                                 "currency_id": 2}
                    ...
                    credit_journal_item_3 = {
                         ...
                         'currency_id': item[0],
                         'analytic_account_id': 100,
                         'credit': float(item[10])
                    }
                    journal_items.append(credit_journal_item_3)
                    credit_journal_item_4 = {
                         'move_id': journal_entry_id,
                         'account_id': 5310,
                         'amount_currency': float(item[11]),
                         'currency_id': item[0],
                         'analytic_account_id': 100,
                         'credit': float(item[12])
                    }
                    journal_items.append(credit_journal_item_4)
                    dr_cr_journal_item = {
                         'move_id': journal_entry_id,
                         'account_id': 1106,
                         'amount_currency': float(item[13]),
                         'currency_id': item[0],
                         'analytic_account_id': 80,
                         'debit': float(item[14]),
                         'credit': float(item[15])
                    }
...
def main():
    ...
    func_list = ['sync_account_fund_profit_share_safe_float_interest']
