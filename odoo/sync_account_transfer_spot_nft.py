    ...
    def sync_spot_rebate(self):
        ...
        try:
            ...
            spot_sql = """
                ...
                from (select 
                    currency_id, sum(transfer_amount) as trans_num
                from gamefi.gamefi_transfer
                where update_time >= '{}' and update_time < '{}'
                and transfer_status = 1 and transfer_type = 1 and member_id not in (
                    select member_id
