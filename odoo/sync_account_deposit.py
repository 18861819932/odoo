    ...
    def sync_deposit_data(self):
        ...
        try:
            ...
            deposit_sql = """
                select 
                    odoo_currency_id,
                    round(cast(sum(num) as double), 6) as total_num,
                    round(cast(sum(num * to_usd_rate) as double), 6) as total_usd
                from odoo.dwd_odoo_account_deposit
                where stime = '{}'
                group by odoo_currency_id
            """.format(self.stime.date())
            self.__ch_conn.connect()
            result = self.__ch_conn.execute(deposit_sql)
            journal_items = []
            if result['code']:
                ...
                journal_entry = {"date": str(self.stime.date()),
                                 "ref": "User deposit {}-{}".format(self.stime.date(), self.tomorrow.date()),
                                 "journal_id": 50,
                                 "company_id": 24,
                                 "currency_id": 2}
