...
class DataProcess:
    ...
    def sync_fund_collect_lose(self):
        ...
        try:
            ...
            fund_collect_lose_sql = """
               			select  b.id,a.fund_currency_mark,
                        100 as fund_finance_type,
                        round(if(a.fund_finance_type in ('客户资金',''),a.initial_price,0), 6) as  coin_num_client,   
                        round(if(a.fund_finance_type in ('客户资金',''),a.initial_price,0)*c.price_usdt, 6) as usd_num_client,
                        round(if(a.fund_finance_type = '公司资金',a.initial_price,0), 6) as  coin_num_company,   
                        round(if(a.fund_finance_type = '公司资金',a.initial_price,0)*c.price_usdt, 6) as usd_num_company,
                ...
                where a.status in (80,90) and toDate(b.stime) =if(toDate('{}')>='2022-12-01','{}','2022-12-01') and toDate(c.stime) ='{}' and a.run_begin_date ='{}'
            """.format(self.stime.date(), self.stime.date(),self.stime.date(),self.stime.date())
            self.__ch_conn.connect()
            result = self.__ch_conn.execute(fund_collect_lose_sql)
            print(fund_collect_lose_sql)
            print(result['data'])
            if result['code']:
                ...
                for item in result['data']:
                    ...
                    journal_entry = {"date": str(self.stime.date()),
                                 "ref": "<%s> Fund purchase unsuccessful"%(item[1]),
                                 "journal_id": 56,
                                 "company_id": 24,
                                 "currency_id": 2}
                    ...
                    credit_journal_item_1 = {
                         'move_id': journal_entry_id,
                         'account_id': 977,
                         'amount_currency': float(item[5]*-1),
                         'currency_id': item[0],
...
def main():
    ...
    func_list = ['sync_fund_collect_lose']
