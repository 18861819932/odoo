...
# -*- coding: utf-8 -*-
"""
日期    : 2023/1/14 12:28 PM
作者    : wushuang
功能    : 保证金（杠杆）账户 划转 到  现货账户
参数    : 
"""
    ...
    def sync_spot_rebate(self):
        ...
        try:
            ...
            margin_sql = """
                select 
                    b.id,
                    round(cast(a.trans_num * -1 as double), 6) as coin_num,
                    round(cast(a.trans_num * c.to_usd_rate * -1 as double), 6) as usd_num
                from (select 
                        currency_id, sum(change_amount) as trans_num
                    from df01.`ods_iwala_margin_finance`
                    where add_time >= toUnixTimestamp('{}') and add_time < toUnixTimestamp('{}')
                    and `type`  = 120 and status = 1 and member_id not in (
                        select member_id
            ...
            self.log.info("sql execute: {}".format(margin_sql))
            ...
            if result['code']:
                ...
                journal_entry = {"date": str(self.stime.date()),
                                 "ref": "user movement margin to spot",
                                 "journal_id": 52,
                                 "company_id": 24,
                                 "currency_id": 2}
