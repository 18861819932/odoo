...
class DataProcess:
    ...
    def sync_margin_dft_discount(self):
        ...
        try:
            ...
            dft_fee_sql = """
                select      b.id,
                round(cast(dft_fee_num as double), 6) as  coin_num,
                round(cast(cast(dft_fee_num as float)*(select  price_usdt 
                                                   from  df01.ods_r_d_iwala_currency_zero_price 
                                                   where currency_trade_id =132 and toDate(stime) = '{}') as double), 6) as usd_num
                from (select 
                    currency_trade_id,
                    sum(dft_fee) as  dft_fee_num
                FROM df01.ods_iwala_trade_user
                WHERE toDate(add_time) = '{}' 
                  AND kind = 'margin'
                  group by currency_trade_id
                  having dft_fee_num >0) a
            ...
            result = self.__ch_conn.execute(dft_fee_sql)
            journal_items = []
            if result['code']:
                ...
                journal_entry = {"date": str(self.stime.date()),
                                 "ref": "tx fee charged on margin-DFT discount",
                                 "journal_id": 52,
                                 "company_id": 24,
                                 "currency_id": 2}
...
def main():
    ...
    func_list = ['sync_margin_dft_discount']
