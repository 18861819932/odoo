...
class DataProcess:
    ...
    def sync_account_fund_caculate_safe_float_interest(self):
        ...
        try:
            ...
            fund_caculate_risk_float_interest_sql = """
                                            ...
                                            where  a.fund_type = '不保本浮息' and toDate(b.stime) =if(toDate('{}')>='2022-12-01','{}','2022-12-01') and toDate(c.stime) ='{}' and a.record_date ='{}'and a.record_date = e.record_date)
                                        """.format(self.stime.date(),self.stime.date(),self.stime.date(),self.stime.date())
            self.__ch_conn.connect()
            result = self.__ch_conn.execute(fund_caculate_risk_float_interest_sql)
            print(fund_caculate_risk_float_interest_sql)
            print(result['data'])
            if result['code']:
                ...
                for item in result['data']:
                    ...
                    journal_entry = {"date": str(self.stime.date()),
                                 "ref": "<%s> Float Rate Non-Guaranteed Fund monthly accrual"%(item[1]),
                                 "journal_id": 56,
                                 "company_id": 24,
                                 "currency_id": 2}
