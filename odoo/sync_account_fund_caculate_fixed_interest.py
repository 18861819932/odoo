...
class DataProcess:
    ...
    def sync_account_fund_caculate_fixed_interest(self):
        ...
        try:
            ...
            fund_caculate_fixed_interest_sql = """
                                                 
	                                            select id,fund_currency_mark,fund_finance_type,coin_num_float_profit,usd_num_float_profit_dr,usd_num_float_profit_cr,coin_num_client_profit,usd_num_client_profit,
	                                                   coin_num_fair_value_change,if(usd_num_fair_value_change_dr>=usd_num_fair_value_change_cr,usd_num_fair_value_change_dr-usd_num_fair_value_change_cr,toDecimal256(0,6)) as usd_num_fair_value_change_d,
	                                            ...
	                                            from (  select  b.id as id,e.fund_currency_mark as fund_currency_mark,
			                                                    ...
			                                                    toDecimal256(-1*a.client_profit*c.price_usdt, 6) as  usd_num_client_profit,
			                                                    -1*(coin_num_float_profit+coin_num_client_profit) as  coin_num_fair_value_change,
			                                                    toDecimal256(if(a.fair_value_change >=  0,a.fair_value_change*c.price_usdt,0), 6)   as  usd_num_fair_value_change_dr,
			                                                    (usd_num_float_profit_dr+usd_num_fair_value_change_dr-usd_num_float_profit_cr-usd_num_client_profit) as  usd_num_fair_value_change_cr
                                            ...
                                            where  a.fund_type = '保本固息' and toDate(b.stime) =if(toDate('{}')>='2022-12-01','{}','2022-12-01') and toDate(c.stime) ='{}' and a.record_date ='{}'and a.record_date = e.record_date)
                                        """.format(self.stime.date(),self.stime.date(),self.stime.date(),self.stime.date())
            self.__ch_conn.connect()
            result = self.__ch_conn.execute(fund_caculate_fixed_interest_sql)
            print(fund_caculate_fixed_interest_sql)
            print(result['data'])
            if result['code']:
                ...
                for item in result['data']:
                    ...
                    journal_entry = {"date": str(self.stime.date()),
                                 "ref": "<%s> Fixed Rate Guaranteed Fund monthly accrual"%(item[1]),
                                 "journal_id": 56,
                                 "company_id": 24,
                                 "currency_id": 2}
...
def main():
    ...
    func_list = ['sync_account_fund_caculate_fixed_interest']
