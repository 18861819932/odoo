    ...
    def sync_expr_fund_pay(self):
        ...
        try:
            ...
            withdraw_gas_sql = """
                select
                    b.id,
                    round(cast(a.fund_pay * -1 as double), 6) as fund_pay_num,
                    round(cast(a.fund_pay * -1 * c.to_usd_rate as double), 6) as fund_pay_usd
                from (select 
                    currency, 
                    sum(available_change) as fund_pay
                from df01.`ods_perpetual_finance` 
                where finance_type = 17 and participant_id = 448038
