...
class DataProcess:
    ...
    # 清理数据
    def clean_data(self):
        clean_sql = "optimize table df02.`dim_currency_exchange_rate_min`"
        self.__ch_conn.connect()
        clean_rst = self.__ch_conn.execute(clean_sql)
    ...
    def load_data(self):
        ...
        if data_len > 0:
            write_sql = "insert into df02.dim_currency_exchange_rate_min VALUES"
            self.__ch_conn.connect()
...
def main():
    ...
    func_list = ['get_exchange', 'load_data']
