...
class DataProcess:
    ...
    def sync_margin_interest(self):
        ...
        try:
            ...
            margin_interest_sql = """
                select          b.id,
                    round(cast(a.interest_money as double), 6) as coin_num,
                    round(cast(if(c.price_usdt = 0 ,a.interest_money * e.price_usdt, a.interest_money *c.price_usdt)
                    as double), 6) as usd_num
                from (	SELECT currency_id, abs(sum(change_amount))as interest_money                                                
						from df01.ods_iwala_margin_finance                                                                                                                                        
						where type = 114 and toDate(FROM_UNIXTIME(add_time)) = '{}'                                                                                                            
                ...
                left join df01.ods_r_d_iwala_currency_zero_price c on a.currency_id = c.currency_trade_id 
            ...
            result = self.__ch_conn.execute(margin_interest_sql)
            journal_items = []
            if result['code']:
                ...
                journal_entry = {"date": str(self.stime.date()),
                                 "ref": "interest charged on margin",
                                 "journal_id": 52,
                                 "company_id": 24,
                                 "currency_id": 2}
...
def main():
    ...
    func_list = ['sync_margin_interest']
