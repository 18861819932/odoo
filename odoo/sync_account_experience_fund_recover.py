...
class DataProcess:
    ...
    def sync_expr_fund_recover(self):
        ...
        try:
            ...
            withdraw_gas_sql = """
                select
                    b.id,
                    round(cast(a.fund_pay as double), 6) as fund_pay_num,
                    round(cast(a.fund_pay * c.to_usd_rate as double), 6) as fund_pay_usd
                from (select 
                    currency, 
                    sum(available_change) as fund_pay
                from df01.`ods_perpetual_finance` 
                where finance_type = 21 and participant_id = 448038
                    and insert_time >= '{}' and insert_time < '{}'
                group by  currency) a 
                join odoo.dim_odoo_currency b on a.currency = b.full_name
                left join df02.dim_currency_exchange_rate c on a.currency = c.currency_mark
...
def main():
    ...
    func_list = ['sync_expr_fund_recover']
